...
abstract class Helper
{
	...
	/**
	 * получаем пользовател€ по токену и каторизуем его в системе
	 * @param string $sToken
	...
	/**
	 * ѕолучаем значение свойства текущего польлзовател€ по его коду
	...
	public static function getProfileFieldValue($sId)
	{
		...
		switch ($sId) {
			...
			case 'sex':
				...
				switch ($sGender) {
					case 'M':
						return GetMessage("MOBIUM_API_MUJSKOY");
					case 'F':
						return GetMessage("MOBIUM_API_JENSKIY");
					default:
						return '';
				}
			case 'email':
	...
	public static function setProfileFieldValue($sId, $mValue)
	{
		...
		switch ($sId) {
			case 'name':
				$USER->SetParam('NAME', $mValue);
			case 'email':
				$USER->SetParam('EMAIL', $mValue);
			case 'last_name':
				$USER->SetParam('LAST_NAME', $mValue);
			case 'phone':
	...
	public static function getProfileKey($sId) {
		switch ($sId) {
			case 'name':
				return "NAME";
			case 'email':
	...
	{
		$aResult = [];
		$aData = static::deliveryTypes();
		if(is_set($aData["error"])) {
			$aResult["023"] = GetMessage("MOBIUM_API_BEZ_DOSTAVKI");
			$aResult["024"] = GetMessage("MOBIUM_API_KURQER");
			$aResult["025"] = GetMessage("MOBIUM_API_SAMOVYVOZ");
		}
		else {
			foreach ($aData as $aItem) {
	...
	public static function getMobiumDeliveryAreas()
	{
		$aResult = [];
		return $aResult;
		if (false !== ($aData = static::deliveryTypes())) {
			foreach ($aData as $aItem) {
	...
	public static function createUserAppProfile($oUser)
	{
		...
		$arProfileFields = array(
			"NAME" => GetMessage("MOBIUM_API_PROFILQ_POKUPATELA") . $oUser->GetFirstName() . '',
			"USER_ID" => $oUser->GetID(),
			"PERSON_TYPE_ID" => 1
		);
		...
		$PROPS = [
			[
				...
				"ORDER_PROPS_ID" => 3,
				"NAME" => GetMessage("MOBIUM_API_TELEFON"),
				"VALUE" => $aUserData['PERSONAL_PHONE']
			],
			[
				"USER_PROPS_ID" => $PROFILE_ID,
				"ORDER_PROPS_ID" => 25,
				"NAME" => GetMessage("MOBIUM_API_F_I_O"),
				"VALUE" => $oUser->GetFullName()
			],
			[
				"USER_PROPS_ID" => $PROFILE_ID,
				"ORDER_PROPS_ID" => 31,
				"NAME" => GetMessage("MOBIUM_API_DISKONTNOY_KARTY"),
	...
	public static function updateProfileForOrder($oUser, $aData)
	{
		...
		foreach ($aData as $sFieldId => $sValue) {
			...
			switch ($sFieldId) {
				case 'city':
					$sName = GetMessage("MOBIUM_API_GOROD");
					$iPropId = 5;
					break;
				case 'building':
					$sName = GetMessage("MOBIUM_API_DOM");
					$iPropId = 27;
					break;
				case 'street':
					$sName = GetMessage("MOBIUM_API_ULICA");
					$iPropId = 26;
					break;
				case 'name':
					$sName = GetMessage("MOBIUM_API_IMA");
					$iPropId = 25;
					break;
				case 'phone':
					$sName = GetMessage("MOBIUM_API_TELEFON");
					$iPropId = 3;
					break;
				case 'location':
					$sName = GetMessage("MOBIUM_API_MESTOPOLOJENIE");
					$iPropId = 6;
					break;
				case 'apartments':
					$sName = GetMessage("MOBIUM_API_KVARTIRA");
					$iPropId = 28;
					break;
				case 'email':
					$sName = 'E-Mail';
	...
	public static function generateAccessToken($iUserId)
	{
		...
		if (count($aResult) === 0) {
			...
			throw new \Exception(GetMessage("MOBIUM_API_NEODNOZNACNYY_TOKEN"));
		}
		return $sToken;
	}
	...
	public static function checkRecaptcha($sRecaptchaToken, $sPlatform)
	{
		...
		switch ($sPlatform) {
			...
			case 'ios':
				...
				break;
			default:
				throw new \Exception(GetMessage("MOBIUM_API_PLATFORMA_NE_RASPOZN"));
		}
		$data = [
		...
		if (json_last_error() !== JSON_ERROR_NONE || $captcha_success->success == false) {
			throw new \Exception(GetMessage("MOBIUM_API_KAPTCA_VVEDENA_NE_PR"));
		}
		return true;
